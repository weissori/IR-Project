...
anchor_index = InvertedIndex.read_index('.', 'title_index')
...
def search():
    ...
    if len(query) == 0:
        return jsonify(res)
    # BEGIN SOLUTION
    res = backend.search(title_index, body_index, anchor_index, page_rank, 0, query)
    res = [(r, title_id[r]) for r in res][:50]
    # END SOLUTION
    print(res)
    return jsonify(res)
