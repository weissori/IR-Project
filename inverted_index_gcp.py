...
class MultiFileWriter:
    ...
    def upload_to_gcp(self):
        ...
        blob = self.bucket.blob(f"body_bins/{file_name}")
...
class MultiFileReader:
    ...
    def __init__(self):
        self._open_files = {}
        self.fs = gcsfs.GCSFileSystem(project= 'ir-project-new')
        self.locs_mapp = {"body": "body_bins", "title": "title_bins", "anchor": "anchor_bins", "title2": "title_bins2"}

    def read(self, locs, n_bytes, idx_type):
        b = []
        for f_name, offset in locs:
            if f_name not in self._open_files:
                self._open_files[f_name] = open(f"{self.locs_mapp[idx_type]}/{f_name}", 'rb')
...
class InvertedIndex:  
    def __init__(self, idx_type, docs={}):
        ...
        # stores document length for each doc-id
        self.DL = {}
    ...
    def read_posting_list(self, w):
        ...
        with closing(MultiFileReader()) as reader:
            ...
            b = reader.read(locs, self.df[w] * TUPLE_SIZE, self.idx_type)
            posting_list = {}
    ...
    def read_posting_list_dict(self, w):
        """ A generator that reads one posting list from disk and yields
            a (word:str, [(doc_id:int, tf:int), ...]) tuple.
        """
        with closing(MultiFileReader()) as reader:
    ...
    def _upload_posting_locs(bucket_id, posting_locs, bucket_name):
        ...
        blob_posting_locs = bucket.blob(f"body_bins/{bucket_id}_posting_locs.pickle")
